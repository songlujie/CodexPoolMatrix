...
import { LayoutDashboard, ListTodo, Settings, ScrollText, User } from 'lucide-react';
...
const navItems = [
  ...
  { label: 'Task Queue', path: '/tasks', icon: ListTodo },
  ...
  return (
    <aside className="w-[240px] shrink-0 border-r border-border/50 bg-background/50 backdrop-blur-md flex flex-col overflow-y-auto">
        ...
        <div className="p-4 border-b border-border/50">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-muted-foreground capitalize">{currentAccount.auth_type} · 当前激活</p>
            </div>
          </div>
        </div>
      ) : (
        <div className="p-4 border-b border-border/50">
          <p className="text-xs text-muted-foreground">暂无激活账号</p>
          <p className="text-[11px] text-muted-foreground mt-0.5">点账号卡片的 ··· → Set Active</p>
        </div>
      )}
      ...
      <div className="p-4 flex-1 min-h-0">
        <h3 className="text-xs font-medium text-muted-foreground mb-3">最近操作</h3>
