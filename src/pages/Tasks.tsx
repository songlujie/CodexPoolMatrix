...
const Tasks = () => {
  ...
  useEffect(() => {
    loadPage().catch((error: Error) => toast.error(formatAppError(error, '加载任务列表失败')));
  }, []);
  ...
  const addTask = async () => {
    ...
    try {
      ...
      toast.error(formatAppError(error, '创建任务失败'));
    }
  };
  ...
  const batchCancel = async () => {
    try {
      ...
      toast.error(formatAppError(error, '取消任务失败'));
    }
  };
  ...
  const batchRetry = async () => {
    try {
      ...
      toast.error(formatAppError(error, '重试任务失败'));
    }
  };
  ...
  return (
    <div className="h-screen flex flex-col">
      <Header activeAccount={currentAccount?.account_id || t('common.none')} mode={settings.mode} onModeChange={(mode) => setSettings((prev) => prev ? { ...prev, mode } : prev)} />
