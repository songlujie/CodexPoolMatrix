...
export const api = {
  ...
  createAccount: (payload: Pick<Account, 'account_id' | 'email' | 'auth_type' | 'auth_file_path'>) => request<Account>('/api/accounts', { method: 'POST', body: JSON.stringify(payload) }),
  ...
  listLogs: (params?: { level?: string; account?: string }) => {
